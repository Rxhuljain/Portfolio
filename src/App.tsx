...
import { Github, Mail, FileText, ExternalLink, ChevronDown, User, Briefcase, Code, Send, Linkedin, Twitter } from 'lucide-react';
  ...
  return (
    <div className="group bg-gray-800/30 rounded-2xl p-8 backdrop-blur-lg border border-gray-700/50 hover:border-blue-500/50 transition-all duration-500">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {tags.map((tag, index) => (
          <span key={index} className="px-3 py-1 bg-blue-500/10 text-blue-400 rounded-full text-sm font-medium">
            {tag}
          </span>
        ))}
      </div>
      <a 
        href={link}
        className="inline-flex items-center gap-2 text-blue-400 hover:text-blue-300 transition-colors group-hover:translate-x-2 transition-transform font-medium"
      >
